...
public abstract class DataTree<T extends DataTreeObject<T>> extends BaseTree {
  private static final long serialVersionUID = 1L;
	 ...
	 */
	public DataTree(String id, HibernateObjectModel<T> rootModel) {
		super(id);
		...
		setDefaultModel(Model.of(new DefaultTreeModel(rootNode)));
	}
	 ...
	 */
	public DataTree(String id, HibernateListModel<T> topLevelModel) {
		super(id);
		setRootLess(true);
	...
	public static class TopLevelCriteriaBuilder implements CriteriaBuilder {
	  private static final long serialVersionUID = DataTree.serialVersionUID;
	...
	public T getDataTreeNode(DefaultMutableTreeNode node) {
		T nodeObject = ((IModel<T>) node.getUserObject()).getObject();
		return (nodeObject instanceof DataTreeObject<?>) ? nodeObject : null;
	}
